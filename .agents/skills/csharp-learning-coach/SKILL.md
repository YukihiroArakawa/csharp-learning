...
9. In Japanese prose, put each sentence on its own source line by inserting a line break after `。`. Do not split fenced code, inline code, tables, URLs, or other structured content where a line break would change meaning or rendering.
10. Include only content that directly supports the exercise objective, implementation steps, observations, or completion criteria. Do not add tangential C#/.NET explanations to the exercise document; answer those questions in chat instead.
11. When optional background is still directly relevant to completing the exercise, place it under a clearly labeled `コラム` heading and keep it out of the completion criteria.
12. Use a Mermaid diagram only when explaining an abstract concept whose relationships, lifecycle, scope, ownership, or control flow materially benefit from a diagram. Put prose first, keep the diagram small, and label every node and arrow in plain language. Do not use raw HTML, inline SVG, or a diagram for concrete code and short procedures.
13. Link the Markdown exercise from the chat and summarize only the immediate starting action.
14. Update an existing exercise document only when the learner explicitly requests the update and the requested content is directly relevant to that exercise. If either condition is not met, answer in chat without changing the document.
15. Create a paired Markdown answer sheet for every new exercise under the same `docs/` directory. Name it `<exercise-base>-answers.md`, such as `01-di-lifetimes-answers.md`. Keep canonical explanations and instructions in the exercise Markdown; the answer sheet contains only the exercise link, the review questions, blank answer sections, and a completion-status field.
16. Link both the exercise Markdown and its answer sheet from the chat. When the learner answers in chat instead of editing the sheet, do not copy the answer into the file unless explicitly requested.
17. Before presenting an exercise, audit every review question against the exercise document. For each question, identify the exact prose, code, or observed result that teaches every fact required for a correct answer. Add the missing explanation or simplify the question if the document alone is insufficient; do not rely on prior chat or unstated inference.
18. Treat the exercise document as the learner's available syllabus, not merely as a reference for the agent. A review question must not test terminology, behavior, rationale, or framework conventions that appear only in generated code or prior chat. When code reading is required, the document must explicitly direct the learner to the relevant file and explain enough context to interpret it.
